...
class UpdateClient:
    ...
    def start(self):
        ...
        if not HAS_REQUESTS:
            logger.warning("⚠️ 'requests' nicht installiert — Update-Client deaktiviert")
            return
    ...
    def _check_and_apply(self):
        ...
        try:
            ...
            if not update_info.get("available"):
                logger.info("✅ Keine Updates verfügbar — System ist aktuell")
                return
            ...
            if success:
                logger.success(f"✅ Update auf {new_version} erfolgreich angewendet")
                ...
                logger.error("❌ Update fehlgeschlagen — führe Rollback durch")
                self._rollback()
    ...
    def _download_update(self, url: str, version: str, expected_checksum: str = None) -> Path | None:
        ...
        try:
            logger.info(f"⬇️ Lade Update herunter: {url}")
            ...
            logger.info(f"✅ Download abgeschlossen: {update_file.stat().st_size / 1024 / 1024:.1f} MB")
            return update_file
    ...
    def _rollback(self):
        ...
        try:
            ...
            logger.success("✅ Rollback erfolgreich")
