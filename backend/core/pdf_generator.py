...
class PDFGenerator:
    ...
    def create_pdf_from_images(
        ...
        """
        try:
            logger.info(f"[DOC] Erstelle PDF: {output_path.name} ({len(image_paths)} Seite(n))")
            ...
            for idx, image_path in enumerate(image_paths):
                if not image_path.exists():
                    logger.error(f"[ERROR] Bild nicht gefunden: {image_path}")
                    continue
                ...
                logger.debug(f"  [OK] Seite {idx + 1}/{len(image_paths)} hinzugefügt")
            ...
            logger.success(f"[OK] PDF erstellt: {output_path.name} ({file_size_kb:.1f} KB)")
            return True
            
        except Exception as e:
            logger.error(f"[ERROR] Fehler bei PDF-Erstellung: {e}")
            return False
    ...
    def _add_text_layer(self, pdf_document: fitz.Document, text: str):
        ...
        try:
            ...
            logger.debug("  [OK] OCR-Text-Layer hinzugefügt")
            
        except Exception as e:
            logger.debug(f"  [WARNING] Konnte OCR-Layer nicht hinzufügen: {e}")
    ...
    def merge_pdfs(self, pdf_paths: List[Path], output_path: Path) -> bool:
        ...
        """
        try:
            logger.info(f"[DOC] Füge {len(pdf_paths)} PDFs zusammen")
            
            merged_pdf = fitz.open()
            
            for pdf_path in pdf_paths:
                if not pdf_path.exists():
                    logger.warning(f"[WARNING] PDF nicht gefunden: {pdf_path}")
                    continue
            ...
            logger.success(f"[OK] PDFs zusammengeführt: {output_path.name}")
            return True
            
        except Exception as e:
            logger.error(f"[ERROR] Fehler beim PDF-Merge: {e}")
            return False
