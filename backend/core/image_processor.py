...
class ImageProcessor:
    ...
    def process(self, image_path: Path, output_path: Path) -> bool:
        ...
        """
        try:
            logger.info(f"🖼️ Verarbeite Bild: {image_path.name}")
            ...
            logger.success(f"✅ Bild verarbeitet: {output_path.name}")
            return True
    ...
    def _detect_and_correct_perspective(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if len(approx) == 4:
                ...
                logger.debug(f"⚠️ Keine 4 Ecken gefunden ({len(approx)} Punkte), überspringe Perspektivkorrektur")
                return original
                
        except Exception as e:
            logger.debug(f"⚠️ Perspektivkorrektur fehlgeschlagen: {e}, nutze Original")
            return image
