...
class DocumentNamer:
    ...
    def _build_prompt(self, ocr_text: str, category: str) -> str:
        """Build filename generation prompt (Mistral [INST] format)."""

        prompt = f"""<s>[INST] Du bist ein Metadaten-Extraktor für Dokumente. Antworte NUR mit JSON.

Extrahiere aus dem OCR-Text:
- date: Datum YYYY-MM-DD (heute wenn nicht erkennbar)
- sender: Absender/Empfänger, max 20 Zeichen, nur ASCII (ä->ae, ö->oe, ü->ue, ß->ss, Leerzeichen->nichts)
- subject: Betreff, max 5 Wörter, nur ASCII

Kategorie: {category}
OCR-Text: {ocr_text[:800]}

Antworte NUR mit diesem JSON:
{{"date": "YYYY-MM-DD", "sender": "Kurzname", "subject": "Betreff max 5 Woerter"}}
[/INST]"""
