...
class LLMRouter:
    ...
    def _init_local_llm(self):
        ...
        try:
            ...
            if self._local_llm.is_available():
                logger.info(f"[OK] Local LLM initialized: {self._local_llm.get_provider_name()}")
            else:
                logger.warning("[WARNING] Local LLM not available (model not found or load failed)")
    ...
    def get_llm(self, task_type: str) -> Optional[BaseLLMProvider]:
        ...
        if task in FAST_TASKS:
            ...
            logger.error(f"[ERROR] No LLM available for task '{task_type}'")
            return None
            ...
            logger.error(f"[ERROR] Local LLM not available for task '{task_type}'")
            return None
