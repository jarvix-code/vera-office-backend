...
class VERAAgent:
    ...
    def _generate_response(self, user_message: str, session_id: str) -> str:
        """Generate response using LLM Router (Fast Cloud or Local) or fallback templates."""
        ...
        # Get appropriate LLM for chat (Fast Cloud preferred, Local fallback)
        ...
        if chat_llm:
            ...
            # Detect if using Qwen (Fast LLM) vs Mistral (Local LLM)
            provider_name = chat_llm.get_provider_name()
            use_qwen = "qwen" in provider_name.lower()
            
            if use_qwen:
                # Use Qwen chat format
                logger.debug("Using Qwen prompt format")
                ...
                stop_sequences = get_qwen_stop_sequences()
            else:
                # Use Mistral format
                ...
                stop_sequences = None  # Mistral defaults
            
            response = chat_llm.generate(
                prompt=prompt,
                max_tokens=300,
                temperature=0.7,
                stop=stop_sequences
        ...
        # Fallback: Template-based responses (works without any LLM)
    ...
    def _template_response(self, user_message: str, stats: Dict) -> str:
        ...
        if any(w in msg_lower for w in ["guten morgen", "morgen vera", "morgen!", "moin"]):
            ...
            if pending > 0:
                return f"Morgen! ☀️ {pending} Dokument{'e' if pending != 1 else ''} warten auf dich."
            return "Morgen! ☀️ Alles aufgeräumt — was steht heute an?"
            ...
            for prefix in ["wo ist ", "such ", "find ", "zeig mir ", "ich brauch "]:
                if prefix in msg_lower:
                    ...
                    if query:
                        ...
                        if results:
                            lines = [f"Hab {len(results)} Treffer gefunden: 📄"]
                            for r in results[:5]:
        ...
        if any(w in msg_lower for w in ["lösch", "entfern", "weg damit", "wegmachen"]):
            return "⚠️ Vorsicht! Aufbewahrungsfristen beachten. Welches Dokument meinst du?"
        ...
        if any(w in msg_lower for w in ["statistik", "übersicht", "wie viele", "was gibt's neues", "status", "was gibt es neues"]):
            ...
            if u > 0:
                lines.append(f"  • ⚠️ {u} noch unkategorisiert")
            if p > 0:
                lines.append(f"  • {p} heute verarbeitet")
            if u == 0 and t > 0:
                lines.append("  • ✅ Alles aufgeräumt!")
            return "\n".join(lines)
        
        # Hilfe
        if any(w in msg_lower for w in ["hilfe", "help", "was kannst du", "wie geht", "was machst du"]):
            return ("Ich kann: 🔍 Dokumente suchen, 🗂️ Ablegen & sortieren, "
                    "📊 Statistiken zeigen, ⚠️ An Fristen erinnern. Was brauchst du?")
    ...
    def _generate_suggestions(self, user_message: str) -> List[str]:
        ...
        if any(w in msg_lower for w in ["feierabend", "tschüss", "bye"]):
            return ["Offene Dokumente?", "Alles erledigt ✅", "Bis morgen!"]
