...
class OnboardingConversation:
    ...
    def __init__(self, session_data: Optional[Dict] = None):
        ...
        self.current_state = OnboardingState(self.data.get("current_state", OnboardingState.LANGUAGE))
    
    def get_message(self) -> str:
        """Get current state message."""
        if self.current_state == OnboardingState.GREETING:
            # Legacy: redirect to LANGUAGE
            return (
                "Hallo! Ich bin VERA — deine persönliche Assistentin für Dokumentenmanagement.\n\n"
                "In welcher Sprache möchtest du mit mir kommunizieren?"
            )
        ...
        elif self.current_state == OnboardingState.CERT_INSTALL:
            return (
                "Damit alles reibungslos funktioniert — zum Beispiel die Kamera zum Scannen — "
                "braucht dein Gerät mein Sicherheitszertifikat.\n\n"
                "Tippe auf den Button, um es herunterzuladen und zu installieren."
            )

        elif self.current_state == OnboardingState.USER_NAME:
            return "Super! Wie heißt du?"

        elif self.current_state == OnboardingState.PASSWORD:
            user_name = self.data.get("user_name", "")
            greeting = f"Schön, {user_name}! " if user_name else ""
            return (
                f"{greeting}Lege jetzt dein Master-Passwort fest.\n\n"
                "Es muss mindestens 8 Zeichen lang sein."
            )

        elif self.current_state == OnboardingState.PIN:
            return (
                "Fast geschafft! Wähle eine 4-stellige PIN.\n\n"
                "Du kannst sie später für den schnellen Zugriff auf VERA verwenden."
            )
    ...
    def get_suggestions(self) -> List[str]:
        ...
        if self.current_state in (OnboardingState.GREETING, OnboardingState.LANGUAGE):
            return ["Deutsch"]

        elif self.current_state == OnboardingState.CERT_INSTALL:
            return []  # Handled by frontend with special UI

        elif self.current_state in (OnboardingState.USER_NAME, OnboardingState.PASSWORD, OnboardingState.PIN):
            return []
    ...
    def process_input(self, user_input: str) -> Dict[str, Any]:
        ...
        # GREETING state: legacy — redirect to LANGUAGE
        if self.current_state == OnboardingState.GREETING:
            self.current_state = OnboardingState.LANGUAGE

        # LANGUAGE state: accept any input / button click → go to CERT_INSTALL
        if self.current_state == OnboardingState.LANGUAGE:
            self.data["language"] = user_input if user_input else "Deutsch"
            self.current_state = OnboardingState.CERT_INSTALL

        elif self.current_state == OnboardingState.CERT_INSTALL:
            # Triggered by cert_done action or any input — just advance
            ...
            self.data["user_name"] = name.title()
            self.current_state = OnboardingState.PASSWORD

        elif self.current_state == OnboardingState.PASSWORD:
            if len(user_input) < 8:
                return {
                    "message": "Das Passwort muss mindestens 8 Zeichen lang sein. Bitte versuche es erneut.",
                    "suggestions": [],
                    "input_type": "password",
                    "completed": False,
                    "data": self.data
                }
            self.data["password"] = user_input
            self.current_state = OnboardingState.PIN

        elif self.current_state == OnboardingState.PIN:
            pin = user_input.strip()
            if not pin.isdigit() or len(pin) != 4:
                return {
                    "message": "Bitte gib eine 4-stellige PIN aus Ziffern ein.",
                    "suggestions": [],
                    "input_type": "pin",
                    "completed": False,
                    "data": self.data
            ...
            return {
                "message": (
                    "Alles eingerichtet! Willkommen bei VERA.\n\n"
    ...
    def get_input_type(self) -> str:
        """Return the input type expected for the current state."""
        mapping = {
            OnboardingState.LANGUAGE: "buttons",
            OnboardingState.GREETING: "buttons",
            OnboardingState.CERT_INSTALL: "cert_install",
            OnboardingState.USER_NAME: "text",
            OnboardingState.PASSWORD: "password",
            OnboardingState.PIN: "pin",
            OnboardingState.COMPLETE: "done",
            OnboardingState.COMPANY_NAME: "text",
            OnboardingState.COMPANY_TYPE: "buttons",
            OnboardingState.EMPLOYEE_COUNT: "buttons",
            OnboardingState.DOCUMENT_TYPES: "buttons",
            OnboardingState.NETWORK: "buttons",
        }
        return mapping.get(self.current_state, "text")
