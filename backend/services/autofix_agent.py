...
def load_config():
    ...
    logger.info("[OK] Telegram config loaded")
    return True
...
def send_to_user(telegram_user_id: int, message_text: str) -> bool:
    ...
    try:
        ...
        logger.error(f"[ERROR] Telegram send failed to {telegram_user_id}: {e}")
        return False
...
def backup_file(filepath: Path):
    ...
    logger.debug(f"[SAVE] Backup created: {backup_path.name}")
...
def create_fix_briefing(bug_data: dict) -> Optional[Path]:
    ...
    logger.info(f"[CONFIG] Creating fix briefing for ticket #{ticket_id}...")
    ...
    for file_path in affected_files[:5]:  # Max 5 files
        ...
        if code_data.get("exists"):
            status = " TRUNCATED" if code_data.get("truncated") else "[OK]"
    ...
    logger.info(f"[OK] Fix briefing created: {task_file.name}")
    return task_file
...
def process_bug_queue():
    ...
    logger.info(f"[LIST] Found {len(bug_files)} pending bug(s)")
    
    for bug_file in bug_files:
        try:
            process_single_bug(bug_file)
        except Exception as e:
            logger.error(f"[ERROR] Failed to process {bug_file.name}: {e}", exc_info=True)
...
def process_single_bug(bug_file: Path):
    ...
    if user_id:
        send_to_user(
            user_id,
            f"[OK] <b>Bug #{ticket_id} — Fix-Plan erstellt</b>\n\n"
    ...
    logger.info(f"[OK] Bug #{ticket_id} successfully processed")
...
def run_daemon():
    ...
    # Load config
    if not load_config():
        logger.error("[ERROR] Configuration failed — exiting")
        return
    ...
    logger.info("[OK] Daemon ready — watching queue...")
    ...
    try:
        ...
        logger.error(f"[ERROR] Fatal error in daemon loop: {e}", exc_info=True)
