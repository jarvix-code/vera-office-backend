...
class TelemetryClient:
    ...
    def __init__(self, config_dict: dict):
        ...
        if not self.enabled:
            logger.info("[STATS] Telemetrie deaktiviert (via Config)")
        else:
            logger.info(f"[STATS] Telemetrie-Client initialisiert (Device-ID: {self.device_id}, Intervall: {self.heartbeat_interval_minutes} min)")
    ...
    def _get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"[WARNING] Fehler beim Sammeln von System-Status: {e}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        logger.debug("[OK] Heartbeat gesendet")
                        return True
                    else:
                        logger.warning(f"[WARNING] Heartbeat fehlgeschlagen: HTTP {response.status}")
                        return False
        
        except aiohttp.ClientError as e:
            logger.debug(f"[WARNING] Telemetrie-Server nicht erreichbar: {e}")
            return False
        except Exception as e:
            logger.warning(f"[WARNING] Fehler beim Senden des Heartbeats: {e}")
            return False
    ...
    async def send_event(
        ...
        if len(self._event_buffer) > self.offline_buffer_max:
            logger.warning(f"[WARNING] Telemetrie-Puffer voll ({self.offline_buffer_max}) - älteste Events werden verworfen")
    ...
    async def _flush_events(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"[STATS] Sende {len(batch)} Telemetrie-Events")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.debug(f"[OK] {len(batch)} Events gesendet")
                        return True
                    else:
                        logger.warning(f"[WARNING] Event-Upload fehlgeschlagen: HTTP {response.status}")
                        return False
            ...
            logger.warning(f"[WARNING] Fehler beim Senden von Events: {e}")
            return False
    ...
    async def _background_heartbeat_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.error(f"[ERROR] Fehler im Heartbeat-Loop: {e}")
                await asyncio.sleep(60)  # Retry nach 1 Minute
    
    async def start(self):
        """Startet Background-Task für Heartbeats"""
        if not self.enabled:
            logger.info("[STATS] Telemetrie deaktiviert - kein Background-Task")
            return
        
        if self._running:
            logger.warning("[WARNING] Telemetrie-Client läuft bereits")
            return
        
        self._running = True
        self._task = asyncio.create_task(self._background_heartbeat_loop())
        logger.info("[OK] Telemetrie-Client gestartet")
    ...
    async def stop(self):
        ...
        if self._event_buffer:
            logger.info(f"[STATS] Sende verbleibende {len(self._event_buffer)} Events...")
            await self._flush_events()
        ...
        logger.info("[OK] Telemetrie-Client gestoppt")
