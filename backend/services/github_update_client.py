"""
VERA Office - GitHub Release Update Client
Checkt GitHub Releases für Updates (Entwicklermodus)
"""
import asyncio
import hashlib
import json
...
class GitHubUpdateClient:
    ...
    async def download_update(self, version: str) -> Optional[Path]:
        ...
        try:
            async with httpx.AsyncClient(timeout=300.0, follow_redirects=True) as client:
                logger.info(f"⬇️ Lade Update {version} herunter...")
                ...
                logger.success(f"✅ Update heruntergeladen: {update_file.name} ({len(resp.content) / 1024 / 1024:.1f} MB)")
                return update_file
    ...
    def create_backup(self):
        ...
        logger.success("✅ Backup erstellt")
        return backup_path
