...
def setup_modules(app: FastAPI) -> ModuleRegistry:
    ...
    # 2b. PromoStore erstellen
    promo_store = PromoStore(promo_codes_path)
    logger.info(f"PromoStore initialisiert: {promo_codes_path}")

    # 3. ModuleRegistry erstellen
    registry = ModuleRegistry(license_store, promo_store)
    ...
    try:
        ...
        logger.success("[OK] ERP-Modul registriert")
    except ImportError as e:
        logger.warning(f"[WARNING] ERP-Modul nicht verfügbar: {e}")
    
    # QM-Modul registrieren
    try:
        from backend.modules.qm import qm_module
        registry.register(qm_module)
        logger.success("[OK] QM-Modul registriert")
    except ImportError as e:
        logger.warning(f"[WARNING] QM-Modul nicht verfügbar: {e}")
    ...
    logger.success("[OK] Modul-System initialisiert")
