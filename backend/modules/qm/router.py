...
from .kiqm_import import router as kiqm_router
router.include_router(kiqm_router, tags=['KIQM Import'])
...
# ============================================================
# BLZK Portal Integration (Phase 1: Credentials + Login + Download)
# ============================================================

@router.get("/blzk/credentials")
async def get_blzk_credentials():
    """Gibt BLZK-Credential-Status zurück (ohne Passwort!)."""
    from .blzk.credentials import get_credentials_status
    return get_credentials_status()


@router.post("/blzk/credentials")
async def save_blzk_credentials(data: dict):
    """Speichert BLZK-Zugangsdaten (verschlüsselt)."""
    from .blzk.credentials import save_credentials
    username = data.get("username", "").strip()
    password = data.get("password", "").strip()
    if not username or not password:
        raise HTTPException(status_code=400, detail="Username und Passwort erforderlich")
    return save_credentials(username, password)


@router.delete("/blzk/credentials")
async def delete_blzk_credentials():
    """Löscht gespeicherte BLZK-Zugangsdaten."""
    from .blzk.credentials import delete_credentials
    return delete_credentials()


@router.post("/blzk/login")
async def blzk_login(data: dict = None):
    """
    Test-Login beim BLZK-Portal.
    Nutzt gespeicherte Credentials oder übergebene.
    """
    from .blzk.credentials import load_credentials
    from .blzk.scraper import blzk_scraper
    ...
    if data and data.get("username"):
        ...
        if not creds:
            raise HTTPException(status_code=400, detail="Keine BLZK-Zugangsdaten konfiguriert")
        username = creds["username"]
        password = creds["password"]

    async with blzk_scraper:
        result = await blzk_scraper.login(username, password)
    return result


@router.get("/blzk/documents")
async def get_blzk_portal_documents(area: str = None):
    """Holt Dokument-IDs vom BLZK-Portal (erfordert Login)."""
    from .blzk.credentials import load_credentials
    from .blzk.scraper import blzk_scraper
    ...
    async with blzk_scraper:
        login_result = await blzk_scraper.login(creds["username"], creds["password"])
        if not login_result.get("success"):
            raise HTTPException(status_code=401, detail=login_result.get("error", "Login fehlgeschlagen"))
        return await blzk_scraper.get_document_ids(area)


@router.post("/blzk/download")
async def download_blzk_documents(data: dict):
    """
    Lädt Dokumente vom BLZK-Portal als PDF-Bundle herunter.
    
    Body: {"doc_ids": [...], "filename": "optional_name"}
    """
    from .blzk.credentials import load_credentials
    from .blzk.scraper import blzk_scraper
    ...
    async with blzk_scraper:
        ...
        if not login_result.get("success"):
            raise HTTPException(status_code=401, detail=login_result.get("error", "Login fehlgeschlagen"))
        return await blzk_scraper.download_bundle(doc_ids, filename)


# ============================================================
# BLZK Requirements Mapping
# ============================================================

@router.get("/blzk/requirements")
async def get_blzk_requirements_list(keyword: str = None):
    """
    Alle BLZK Requirements oder Suche nach Keyword.
    """
    from .blzk.requirements import get_all_requirements, search_by_keyword
    if keyword:
        return {"results": search_by_keyword(keyword), "keyword": keyword}
    return {"requirements": get_all_requirements()}


@router.get("/blzk/requirements/{question_id}")
async def get_blzk_requirement(question_id: str):
    """Gibt BLZK-Anforderungen für eine bestimmte Frage zurück."""
    from .blzk.requirements import get_requirements
    req = get_requirements(question_id)
    if not req:
        raise HTTPException(status_code=404, detail=f"Keine Anforderungen für {question_id}")
    return req
