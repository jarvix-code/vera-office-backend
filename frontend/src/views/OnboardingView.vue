    ...
    <q-card class="onboarding-card" flat bordered>
      ...
      <q-card-section>
        ...
        <div v-else-if="displayStep === 'ssl'">
          ...
          <!-- Mobileconfig install (iOS preferred path) -->
          <q-card flat bordered class="q-mb-lg bg-green-1">
            <q-card-section>
              <div class="text-subtitle1 text-weight-bold q-mb-xs">
                <q-icon name="verified_user" color="positive" size="sm" class="q-mr-xs" />
                Empfohlen: Sicherheitszertifikat installieren
              </div>
              <div class="text-body2 text-grey-8 q-mb-md">
                Tippen Sie auf den Button unten — iOS oeffnet automatisch den Profil-Installer.
                Danach entfaellt die Sicherheitswarnung dauerhaft.
              </div>
              <q-btn
                unelevated
                color="positive"
                icon="download"
                label="Sicherheitszertifikat installieren"
                :href="`${veraOrigin}/vera.mobileconfig`"
                target="_blank"
                class="full-width"
                style="min-height: 52px; font-size: 15px;"
              />
              <div class="text-caption text-grey-6 q-mt-sm text-center">
                Nach der Installation: Einstellungen → Allgemein → VPN & Geräteverwaltung → Profil vertrauen
              </div>
            </q-card-section>
          </q-card>
          ...
          <q-banner class="bg-amber-2 q-mb-md" rounded>
            ...
            <div class="text-subtitle1 text-weight-bold q-mb-sm">Ohne Zertifikat: Warnung manuell bestaetigen</div>
            <div class="text-body2">
              Falls Sie das Zertifikat nicht installieren moechten, koennen Sie die Warnung
              einmalig manuell bestaetigen — das ist fuer lokale Netzwerke sicher.
            </div>
          </q-banner>
...
// Origin for mobileconfig link — works whether accessed via IP or hostname
const veraOrigin = computed(() => window.location.origin)
